...
def get_mean_attention_map(attn, token, shape):
    ...
    attn = attn.unflatten(2, torch.Size([shape[2] // 16,
                                         shape[3] // 16])).float()
    attn = torch.nn.functional.interpolate(attn,
                                           size=shape[2:],
                                           mode="bicubic",
...
def _make_pretrained_clip_vitl16_384(pretrained,
    ...
    clip_pretrained, clip_preprocess = clip.load("ViT-B/32",
                                                 device="cuda",
                                                 jit=False)
    ...
    return clip_pretrained, pretrained, clip_preprocess


def _make_pretrained_clipRN50x16_vitl16_384(pretrained,
                                            use_readout="ignore",
                                            hooks=None,
                                            enable_attention_hooks=False):
    clip_pretrained, clip_preprocess = clip.load("RN50x16",
                                                 device="cuda",
                                                 jit=False)
    model = timm.create_model("vit_large_patch16_384", pretrained=pretrained)
...
def _make_pretrained_clip_vitb32_384(pretrained,
                                     use_readout="ignore",
                                     hooks=None,
                                     enable_attention_hooks=False):
    clip_pretrained, clip_preprocess = clip.load("ViT-B/32",
...
def _make_vit_b32_backbone(
    ...
    pretrained.activations = activations  # type: ignore
    ...
    if enable_attention_hooks:
        ...
        pretrained.attention = attention  # type: ignore
