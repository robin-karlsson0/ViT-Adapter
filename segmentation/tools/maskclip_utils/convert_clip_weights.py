...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Extract and save the CLIP visual weights')
    ...
    parser.add_argument(
        ...
        action='store_true',
        help=
        'Prepend the word backbone to the key so that it can be directly loaded as a checkpoint'
    )
...
if __name__ == '__main__':
    ...
    for key in state_dict.keys():
        if 'ViT' in args.model and prefix in key:
            ...
            result_model['state_dict'].update(
                {new_key: state_dict[key].float()})
        elif prefix in key:
            if 'attnpool' in key:
                if 'proj' in key:
                    ...
                    if proj_name not in all_model:
                        all_model[proj_name] = {}
                    all_model[proj_name][weight_or_bias] = state_dict[
                        key].float()
            else:
                new_key = key[len(f'{prefix}.'):]
                if 'layer' not in new_key:
                    layer_name, layer_type = new_key.split('.')
                    new_key = 'stem.{}.{}'.format(stem_mapping[layer_name],
                                                  layer_type)
    ...
    if args.backbone:
        ...
        torch.save(all_model,
                   'pretrain/{}_clip_weights.pth'.format(args.model))
