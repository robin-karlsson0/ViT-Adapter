...
from .mask_sampling_result import MaskSamplingResult, MaskSamplingResultVL
...
@BBOX_SAMPLERS.register_module()
class MaskPseudoSamplerVL(BaseSampler):
    """A pseudo sampler that does not do sampling actually."""
    ...
    def sample(self, assign_result, masks, gt_masks, **kwargs):
        ...
        pos_inds = torch.nonzero(assign_result.gt_inds >= 0,
                                 as_tuple=False).squeeze(-1).unique()
        neg_inds = torch.nonzero(assign_result.gt_inds == -1,
                                 as_tuple=False).squeeze(-1).unique()
        gt_flags = masks.new_zeros(masks.shape[0], dtype=torch.uint8)
        sampling_result = MaskSamplingResultVL(pos_inds, neg_inds, masks,
                                               gt_masks, assign_result,
                                               gt_flags)
        return sampling_result