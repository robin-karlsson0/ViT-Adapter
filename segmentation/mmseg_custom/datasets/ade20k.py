...
class ADE20KVLDataset(CustomDataset):
    """"""
    ...
    def __init__(self,
                 ...
                 index_ade20k_path='data/ADE20K_2021_17_01/index_ade20k.pkl',
                 **kwargs):
        # ADE20K index dict
        with open(index_ade20k_path, 'rb') as f:
            self.index_ade20k = pickle.load(f)

        # Class txt and RGB lists
        self.CLASSES = []
        self.PALETTE = []
        for cls_txt in self.index_ade20k['objectnames']:
            self.CLASSES.append(cls_txt)
            rgb = np.random.randint(0, 256, size=3)
            rgb = list(rgb)
            self.PALETTE.append(rgb)

        super(ADE20KVLDataset, self).__init__(img_suffix='.jpg',
                                              seg_map_suffix='.npz',
