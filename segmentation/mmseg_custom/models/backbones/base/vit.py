...
class TransformerEncoderLayer(BaseModule):
    ...
    def forward(self, x, return_qkv=False):
        ...
        if return_qkv:
            y = self.norm1(x)
            y = F.linear(y, self.attn.attn.in_proj_weight,
                         self.attn.attn.in_proj_bias)
            N, L, C = y.shape
            y = y.view(N, L, 3, C // 3).permute(2, 0, 1,
                                                3).reshape(3 * N, L, C // 3)
            y = F.linear(y, self.attn.attn.out_proj.weight,
                         self.attn.attn.out_proj.bias)
...
class VisionTransformerVL(BaseModule):
    """Vision Transformer.
    ...
    def init_weights(self):
        if (isinstance(self.init_cfg, dict)
            ...
            # Ex: 'backbone.pos_embed --> pos_embed
            ...
            super(VisionTransformerVL, self).init_weights()
        else:
    ...
    def train(self, mode=True):
        super(VisionTransformerVL, self).train(mode)
