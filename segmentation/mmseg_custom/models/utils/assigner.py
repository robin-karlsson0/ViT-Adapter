...
@MASK_ASSIGNERS.register_module()
class MaskHungarianAssignerVL(BaseAssigner):
    """Computes one-to-one matching between predictions and ground truth for
    mask.
    ...
    def assign(self,
        ...
        Args:
            mask_pred (Tensor): Predicted mask, shape [num_query, h, w]
            cls_pred (Tensor): Embedding predictions (100, D)
            gt_masks (Tensor): Ground truth mask, shape [num_gt, h, w].
            gt_labels (Tensor): Label of `gt_masks`, shape (num_gt,).
            img_meta (dict): Meta information for current image.
            gt_masks_ignore (Tensor, optional): Ground truth masks that are
        ...
        """
        ##############################################
        #  Generate target embeddings from idx list
        ##############################################
        # Create an iterable list with 'ignore' idx' removed
        gt_labels_list = gt_labels.tolist()
        # while self.ignore_emb_idx in gt_labels:
        #     gt_labels.remove(self.ignore_emb_idx)

        # Create a tensor of embedding labels (N, D)
        emb_labels = [self.idx_star2emb[idx] for idx in gt_labels_list]
        emb_labels = torch.cat(emb_labels)
        device = cls_pred.device
        emb_labels = emb_labels.to(device)

        assert gt_masks_ignore is None, \
            'Only case when gt_masks_ignore is None is supported.'
        num_gts = gt_labels.shape[0]
        num_queries, emb_dim = cls_pred.shape

        # 1. assign -1 by default
        assigned_gt_inds = cls_pred.new_full((num_queries, ),
                                             -1,
                                             dtype=torch.long)
        assigned_labels = cls_pred.new_full((num_queries, emb_dim),
                                            -1,
                                            dtype=torch.float)
        if num_gts == 0 or num_queries == 0:
            # No ground truth or boxes, return empty assignment
        ...
        # classification and maskcost.
        if self.cls_cost.weight != 0 and cls_pred is not None:
            cls_cost = self.cls_cost(cls_pred, emb_labels)
        else:
            cls_cost = 0
        ...
        # Assign each GT region 'n' to some query 'q'
        matched_row_inds, matched_col_inds = linear_sum_assignment(cost)
        matched_row_inds = torch.from_numpy(matched_row_inds).to(
            cls_pred.device)
        matched_col_inds = torch.from_numpy(matched_col_inds).to(
        ...
        # 4. assign backgrounds and foregrounds
        # assign all indices to backgrounds first
        # assigned_gt_inds[:] = -1  # (100)
        # assign foregrounds based on matching results
        # [query_idx] --> region_idx (0 if no region)
        assigned_gt_inds[matched_row_inds] = matched_col_inds  # + 1
        # assigned_labels[matched_row_inds] = gt_labels[matched_col_inds]
        assigned_labels[matched_row_inds] = emb_labels[matched_col_inds]
        return AssignResult(num_gts, assigned_gt_inds, labels=assigned_labels)