...
class Mask2FormerHeadVL(BaseDecodeHead):
    """
    ...
    def _get_target_single(self, cls_score, mask_pred, gt_labels, gt_masks,
        ...
        Args:
            cls_score (Tensor): (100, D) Mask score logits from a single decoder layer
        ...
        """
        # sample points
        num_queries, emb_dim = cls_score.shape
        ...
        label_emb = sampling_result.pos_gt_labels
        labels = torch.zeros_like(cls_score)
        labels[pos_inds] = label_emb[sampling_result.pos_assigned_gt_inds]
    ...
    def loss_single(self, cls_scores, mask_preds, gt_labels_list,
        ...
        Args:
            cls_scores (Tensor): (B, 100, D) Mask score logits from a single
                decoder layer for all images.
            mask_preds (Tensor): Mask logits for a pixel decoder for all
                images. Shape (batch_size, num_queries, h, w).
            gt_labels_list: [(N)] List of tensors w. idx for each region n.
            gt_masks_list:  [(N,h,w)] List of tensors w. boolean region masks.
        ...
        # emb_maps = emb_maps.flatten(0, 1)
        # labels = labels.flatten(0, 1)
        # label_weights = label_weights.flatten(0, 1)

        # class_weight = cls_scores.new_tensor(self.class_weight)
        target_type = labels.new_ones(labels.shape[:2])
        loss_cls = self.loss_cls(cls_scores, labels, target_type)
    ...
    def loss(self, all_cls_scores, all_mask_preds, gt_labels_list,
        ...
        Args:
            ...
            emb_map (Tensor): VL embedding map (B, D, h, w).
